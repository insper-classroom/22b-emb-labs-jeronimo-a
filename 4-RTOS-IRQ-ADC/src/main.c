...
static void task_proc(void *pvParameters) {
	...
	while (1) {
		...
		// calcula a media dos ultimos 10 valores
		unsigned short media = 0;
		for (int i = 0; i < 10; i++) {
			adcData data;
			if (xQueueReceive(xQueueADC, (void *)&data, 1000)) {
				media += (unsigned short) data.value;
				printf("task_proc: recebeu %d\n", data.value);
			} else {
		...
		xQueueSend(xQueuePROC, (void *)&media, 0);
	}
}
...
static void task_adc(void *pvParameters) {
  ...
  // variável para recever dados da fila
  unsigned short proc_data;

  while (1) {
    if (xQueueReceive(xQueuePROC, &(proc_data), 1000)) {
      printf("task_adc: %d \n", proc_data);
    } else {
      printf("Nao chegou um novo dado em 1 segundo");
    }
...
int main(void) {
	...
	xQueuePROC = xQueueCreate(100, sizeof(unsigned short));
	if (xQueuePROC == NULL) {
