...
#define DW_BUT_PIO				PIOD						// periferico que controla o SW 1 no modulo OLED
#define DW_BUT_PIO_ID			ID_PIOD						// ID do periferico PIOD
#define DW_BUT_IDX				28							// ID do pino conectado ao SW 1 do modulo OLED
#define DW_BUT_IDX_MASK			(1 << DW_BUT_IDX)			// mascara para controlarmos o SW 1 do modulo OLED
...
SemaphoreHandle_t xSemaphoreDwBut;
...
void but_callback(void) {
	printf("IRQ but ok\n");
	uint32_t change = 100;
	xQueueSend(xQueueLedFreq2, (void *)&change, (TickType_t) 10);
...
static void BUT_init(void) {
  ...
  NVIC_EnableIRQ(DW_BUT_PIO_ID);
  NVIC_SetPriority(DW_BUT_PIO_ID, 4); // Prioridade 4
}
